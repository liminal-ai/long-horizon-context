export function classifyToolResult(input) {
  ...
  const promptMode = promptModeFor(operationClass, responseShape, input.outcome);
...
function classifyOperation(toolName, toolInput, rawOutput) {
  ...
  if (toolName === "bash") {
    ...
    if (/\b(vitest|tsx --test|node --test|npm test|pnpm test|pnpm run verify|tsc|typecheck|lint)\b/.test(command)) return "verification";
    ...
    if (/\b(diff --git|Test Files|Tests\s+\d|Command exited with code|✔|✖)\b/.test(rawOutput)) return "command";
    return "command";
  }
  return "unknown";
}

function classifyShape(toolName, outcome, rawOutput, operationClass) {
  ...
  if (/\bENOENT\b|No such file or directory|command not found|invalid option|Cannot find package/i.test(output)) return "simple_failure";
  if (operationClass === "verification" && /\b(Test Files|Tests|failed|passed|vitest|tsc --noEmit|typecheck|lint|✔|✖|CACError|AssertionError|TAP version|# tests)\b/i.test(output)) return "test_result";
  ...
  if (/^diff --git\b/m.test(output) || /\|\s+\d+\s+[+\-]+/m.test(output) || /files? changed,\s+\d+ insertions?/i.test(output)) return "diff_output";
  if (operationClass === "search_or_listing" && (/^\d+:/m.test(output) || /^[^\n:]+:\d+:/m.test(output) || output.length > 0)) return "search_result";
...
function promptModeFor(operationClass, responseShape, outcome) {
...
function parseFacts(input) {
  ...
  return removeNullish({
    ...
    command: typeof input.toolInput?.command === "string" ? input.toolInput.command : null,
    exitCode: parseExitCode(rawOutput),
    targetPath: parsePrimaryPath(input.toolName, rawOutput),
    ...
    searchNoMatches: input.operationClass === "search_or_listing" && input.responseShape === "search_result" ? isSearchNoMatchOutput(rawOutput.trim()) : null,
    searchMatchCount: input.operationClass === "search_or_listing" && input.responseShape === "search_result" ? parseSearchMatchCount(input.operationClass, rawOutput) : null,
    searchMatches: input.operationClass === "search_or_listing" && input.responseShape === "search_result" ? parseSearchMatches(input.operationClass, rawOutput).slice(0, 12) : [],
...
function looksLikeTestRunnerOutput(output) {
  return /\bTAP version\b/i.test(output) && /#\s*tests\s+\d+/i.test(output);
}

function isStructuredReceiptOutput(output) {
...
function isSearchNoMatchOutput(output) {
  const contentLines = output.split(/\r?\n/)
    .map((line) => line.trim())
    .filter(Boolean)
...
function searchContentLines(output) {
...
function parseSearchMatchCount(operationClass, output) {
...
function parseSearchMatches(operationClass, output) {
  if (operationClass !== "search_or_listing") return [];
  return searchContentLines(output)
    .map((line) => {
      let match = line.match(/^([^:]+):(\d+):(.*)$/);
      if (match) return { path: match[1], line: Number(match[2]), text: match[3].trim() };
      match = line.match(/^(\d+):(.*)$/);
      if (match) return { line: Number(match[1]), text: match[2].trim() };
      return { text: line };
    });
}

function parseTestSummary(operationClass, output) {
  ...
  if (testsLine) summary.countLine = testsLine;
  ...
  if (uniqueFailedTests.length) summary.failedTests = uniqueFailedTests.slice(0, 8);
  if (summary.failed === undefined && uniqueFailedTests.length) summary.failed = uniqueFailedTests.length;
  if (summary.passed === undefined) {
    const passMarkers = lines.filter((line) => /^[✔✓]\s+/.test(line)).length;
    if (passMarkers) summary.passed = passMarkers;
  }
  if (summary.total === undefined && summary.passed !== undefined && summary.failed !== undefined) {
    summary.total = summary.passed + summary.failed;
  }

  const assertionLines = lines.filter((line) => /AssertionError|Expected values|strictly deep-equal|actual|expected/i.test(line));
  if (assertionLines.length) summary.assertionLines = assertionLines.slice(0, 8);

  const commandFailure = output.match(/Command exited with code\s+(\d+)/i);
  if (commandFailure) summary.exitCode = Number(commandFailure[1]);
  return Object.keys(summary).length ? summary : null;
}

function parseSubtoolResults(operationClass, output) {
  ...
  return output.split(/\r?\n/)
    ...
    .map((line) => {
      ...
      return removeNullish({ label: match[1], outcome: match[2].toLowerCase(), detail: match[3] ?? null });
    });
}
...
function parseBlockCount(output) {
  const match = output.match(/Successfully replaced\s+(\d+)\s+block\(s\)/i);
  return match ? Number(match[1]) : null;
}

function parseMatchCount(output) {
  const match = output.match(/Found\s+(\d+)\s+occurrences/i);
  return match ? Number(match[1]) : null;
}

function parseMutationDetailsAvailable(toolName, output) {
...
function parseFailedField(output) {
  ...
  if (!occurrenceMatch) return null;
  return uniqueFieldMatch ? `${occurrenceMatch[1]}.${uniqueFieldMatch[1]}` : occurrenceMatch[1];
}

function parseRequiredCondition(output) {
  const uniqueFieldMatch = output.match(/Each\s+([^\s]+)\s+must be unique/i);
  return uniqueFieldMatch ? `${uniqueFieldMatch[1]} must match exactly one location` : null;
}

function parseRetryGuidance(output) {
  if (/provide more context/i.test(output)) return "retry with more surrounding context so the replacement target is unique";
  if (/command not found/i.test(output)) return "install the command or invoke it through the project package runner";
  if (/No such file or directory|ENOENT/i.test(output)) return "check the path or run from the expected working directory";
  return null;
}

function parseMissingCommand(output) {
  const shellMatch = output.match(/(?:sh|bash|zsh):\s*([^:\s]+):\s*command not found/i);
  return shellMatch?.[1] ?? null;
}

function parseSystemError(output) {
  const match = output.match(/\b(ENOENT|EACCES|EPERM|EEXIST|ENOTDIR|EISDIR)\b/);
  return match?.[1] ?? null;
}

function parseFailureType(output) {
...
function parsePrimaryPath(toolName, output) {
  ...
  if (findMatch) return trimPath(findMatch[1]);
  const locationMatch = output.match(/Location of .*?:\s*(.+)/i);
  if (locationMatch) return trimPath(locationMatch[1]);
  return null;
}

function parsePathMentions(output) {
