...
While modal, stdin drives our line editor (printable echo, backspace, ctrl-U kill; kitty CSI-u keys handled) and **pty output is held** — claude keeps running, we delay rendering its bytes (4 MB cap; on overflow the modal cancels with a notice and everything flushes). Enter executes; Esc, ctrl-C, or leader-again cancels; `help`/`?` lists commands; unknown commands print the help line and stay modal.
