...
const mode = process.env.CC_LHC_FAKE_MODE ?? "success";
const systemPrompt = argValue("--system-prompt") ?? "";
const model = argValue("--model") ?? "";

if (mode === "hold-slot") {
  const sentinelPath = process.env.CC_LHC_FAKE_SENTINEL_FILE;
  if (sentinelPath !== undefined && sentinelPath !== "") {
    writeFileSync(sentinelPath, `${Date.now()}\n`, { flag: "a" });
  }
  const ms = Number.parseInt(process.env.CC_LHC_FAKE_SLEEP_MS ?? "200", 10);
...
if (mode === "stdin-file") {
  const outPath = process.env.CC_LHC_FAKE_STDIN_FILE;
  if (outPath === undefined || outPath === "") {
    console.error("CC_LHC_FAKE_STDIN_FILE required");
    process.exit(2);
  }
...
if (mode === "concurrency") {
  const counterPath = process.env.CC_LHC_FAKE_COUNTER_FILE;
  if (counterPath === undefined || counterPath === "") {
    console.error("CC_LHC_FAKE_COUNTER_FILE required");
    process.exit(2);
  }
  let current = 0;
  let peak = 0;
  ...
  await new Promise((resolve) => {
    setTimeout(resolve, Number.parseInt(process.env.CC_LHC_FAKE_SLEEP_MS ?? "200", 10));
  });
  try {
    const raw = readFileSync(counterPath, "utf8");
    const parsed = JSON.parse(raw);
...
if (mode === "sleep") {
  const ms = Number.parseInt(process.env.CC_LHC_FAKE_SLEEP_MS ?? "120000", 10);
  await new Promise((resolve) => {
    setTimeout(resolve, ms);
  });
