...
import {
  acquireSessionOwner,
  SessionOwnerGuardError,
  SessionOwnerLivenessError,
  SessionOwnershipConflictError,
  sessionOwnerGuardPath,
  sessionOwnerPath,
...
describe("session owner lease", () => {
  it("refuses a second live owner and permits ownership after release", () => {
    ...
    expect(() => acquireSessionOwner("session-a", { home, token: "second" })).toThrow(SessionOwnershipConflictError);
    first.release();
  ...
  it("indeterminate liveness fails closed: throws and leaves the lease untouched", () => {
    ...
    ).toThrow(SessionOwnerLivenessError);
  ...
  it("acquisition fails with an actionable error when own identity is unavailable", () => {
    ...
    ).toThrow(/cannot establish process identity for session ownership.*build:native/);
  });
...
describe("session owner acquisition guard (TOCTOU serialization)", () => {
  it("deterministic interleaving: an acquirer arriving mid-reclaim fails at the guard, touching nothing", () => {
    ...
    const staleJson = storedOwnerJson("session-a", "stale", STALE_IDENTITY);
    ...
    const aProbe: ProbeProcessIdentity = (pid) => {
    ...
    const a = acquireSessionOwner("session-a", { home, token: "a", readIdentity: aProbe });
    expect(interleaved).toBe(true);
    // B failed closed at the guard: it read its own identity once and never
    // inspected, deleted, or published a lease.
    expect(bError).toBeInstanceOf(SessionOwnerGuardError);
    ...
    expect(existsSync(sessionOwnerGuardPath("session-a", home))).toBe(false);
  });
  ...
  it("orphaned guard fails closed with an actionable error and is never auto-deleted", () => {
    ...
    const guard = sessionOwnerGuardPath("session-a", home);
    ...
    try {
      acquireSessionOwner("session-a", { home, readIdentity: probe });
    } catch (cause) {
      caught = cause;
    }
    expect(caught).toBeInstanceOf(SessionOwnerGuardError);
    ...
    rmdirSync(guard);
    const lease = acquireSessionOwner("session-a", { home, readIdentity: probe });
  ...
  it("guard is released after success, conflict, and fail-closed liveness errors", () => {
    ...
    const lease = acquireSessionOwner("session-a", { home, readIdentity: selfOnlyProbe(self) });
    expect(existsSync(guard)).toBe(false);

    expect(() => acquireSessionOwner("session-a", { home, readIdentity: selfOnlyProbe(self) })).toThrow(
      SessionOwnershipConflictError,
    );
    expect(existsSync(guard)).toBe(false);
  ...
  it("release verifies the token under the guard and never deletes another owner's record", () => {
    ...
    const lease = acquireSessionOwner("session-a", { home, readIdentity: selfOnlyProbe() });
    const foreign = storedOwnerJson("session-a", "someone-else", STALE_IDENTITY);
  ...
  it("concurrent workers: exactly one acquisition wins, losers fail closed", async () => {
    ...
    try {
      ...
      for (const loss of losses) {
        expect(["LOST SessionOwnershipConflictError", "LOST SessionOwnerGuardError"]).toContain(loss);
      }
      // The surviving lease belongs to the single winner, and no guard leaked.
      const winnerToken = wins[0]!.slice("WON ".length);
      const stored = JSON.parse(readFileSync(sessionOwnerPath("race-session", home), "utf8")) as { token: string };
      expect(stored.token).toBe(winnerToken);
      expect(existsSync(sessionOwnerGuardPath("race-session", home))).toBe(false);
    } finally {
