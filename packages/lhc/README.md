...
| `repairDerivations` | `(input: RepairInput) → OpResult<RepairReceipt>` | Re-derive failed turn compressions, then the chunk summaries they hold up; `limit` caps each list. Runs the instance's inference (call through `sdk.threads`). Sequential by design: the work queue claims head-first. Subjects with queued work are `deferred`; drain and pass again. |
