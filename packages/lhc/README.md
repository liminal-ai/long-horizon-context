...
Fork operations (same domain; composed by the `lhc thread fork` verb below):

| Operation | Signature | Description |
|---|---|---|
| `copyThread` | `(input: CopyThreadInput) → OpResult<CopyThreadReceipt>` | Backup-API copy of a record into a target file and registry under a new id (or `keepThreadId`). Migrates on open, rekeys `thread_metadata`, refuses `path_exists`, `thread_exists`, and `mid_turn` (open turn holds host activity; runtime notes do not count). A `{ threadId }` source lends its registry title and cwd. |
| `hostBinding` / `bindHost` | `(host, sessionId?) → OpResult<HostBinding>` / `(input: BindHostInput) → OpResult<HostBinding>` | Fixed host table: `cc-lhc` and `claude-lhc` bind by registry alias (`claude-code:<id>`, `t3code-lhc:<id>`), `pi-lhc` binds nothing, `codex-lhc` and `grok` bind by file name (`<id>.sqlite`, `grok-<id>.sqlite`) chosen at copy time, so `bindHost` refuses them with `file_bound_host`. |
| `writeIdentityNote` | `(input: IdentityNoteInput) → OpResult<IdentityNoteReceipt>` | One `runtime_note` naming the source thread, home, date, and seat. Lands in the open turn and serves in the tail. |
| `repairDerivations` | `(input: RepairInput) → OpResult<RepairReceipt>` | Re-derive failed turn compressions, then the chunk summaries they hold up; `limit` caps each list. Runs the instance's inference (call through `sdk.threads`). Drains the record's carried work before each pass and runs up to `rounds` passes (default 3) while a pass defers subjects behind work it enqueued; sequential by design, the work queue claims head-first. |
| `forkCompactPlan` / `hostProvider` | `(sourceHost, host, choice?) → ForkCompactPlan` / `(host) → string` | Fixed provider table (`cc-lhc`, `claude-lhc` anthropic; `codex-lhc` openai; `grok` xai; `pi-lhc` its own). A fork across providers compacts first; `choice` forces (`always`) or suppresses (`never`). |
| `handoffCompactOptions` | `(lowerBound?) → { profile: "handoff", params? }` | The compact a cross-provider fork runs: built-in `handoff` profile (full 0 / smooth 40 / detailed 40 / brief 20, `newestClosedProtection` 0), so no closed turn stays raw and a foreign host never renders this host's tool blocks. The open turn (the identity note) is the tail. |
| `forkThreadId` | `(host, { newId?, sessionId? }) → OpResult<string>` | Per-host id policy: `codex-lhc` keys the record by its rollout uuid (thread id = session id, minted when absent); `grok` mints a uuid; registry hosts keep SDK ids. |
| `exportHistory` | `(ref: ThreadRef) → OpResult<HistoryExport>` | Raw history as neutral JSON grouped per turn: `user_prompt`, `assistant_text`, `tool_call`, `tool_result`, `runtime_note` with their blocks (blob bytes excluded). Other kinds are counted in `omitted`. |

`RUST_HOST_THREAD_SCHEMA_VERSION` pins the thread schema the vendored lhc-rs accepts; a test holds it equal to the SDK's current version so a bump fails there rather than at a codex or grok open.

The `lhc` bin (`packages/lhc/bin/lhc`) is a thin wrapper: `lhc thread copy|bind|note|health|repair|export|fork`. `fork` runs copy, identity note, repair, compact (cross-provider, or `--compact`/`--no-compact`), then bind, so a host cannot find the thread before the record is final. One line on stdout, exit 0; a refusal the caller can act on exits 2 with one stderr line; anything else exits 1. `repair` and `fork` derive through `claude -p` (`--claude-bin`, default `~/.local/bin/claude`); `--rounds` passes through. A home is `<dir>/registry.sqlite` plus `<dir>/threads/`.
