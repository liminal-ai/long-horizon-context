...
// through composeTurnRendering then projectLowerBand, hands both derivations back
  ...
  return async (run, item) => {
    ...
    for (const member of members) {
      ...
      if (member.state === "blocked") {
        return sourceDamaged(
          `member ${member.turnId} lower_band_projection blocked while deriving ${kind}`,
        );
      }
      return dependencyNotReady(
        `member_projection_not_ready: member ${member.turnId} lower_band_projection is ${member.state ?? "missing"}`,
      );
    }
