...
export function readMessageDerivationRows(
  ...
  for (const row of raw) {
    ...
    rows.set(composeDerivationKey(row.subject_id, row.derivation_type as DerivationType), view);
  }
  return rows;
}
    ...
    .prepare(
      `SELECT cm.turn_id, t.turn_id AS existing_turn_id, t.deleted_at,
       ...
       LEFT JOIN derivation df ON df.subject_kind = 'turn'
         AND df.subject_id = cm.turn_id AND df.derivation_type = 'lower_band_projection'
