...
import { classifyToolResult } from "../src/messages/internal/classify-tool-result.js";

describe("tool-result classification", () => {
  it("maps tool names and bash commands to operation classes", () => {
    expect(
      classifyToolResult({
        toolName: "read",
        outcome: "succeeded",
  ...
  it("extracts deterministic failure facts for command-not-found output", () => {
  ...
  it("routes receipt and test-shaped responses to prompt modes", () => {
    expect(
      classifyToolResult({
        toolName: "write",
        outcome: "succeeded",
  ...
  it("handles empty and very large unexpected output without throwing", () => {
    expect(() =>
      classifyToolResult({
        toolName: "",
        outcome: "unknown",
