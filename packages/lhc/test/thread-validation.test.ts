...
describe("thread file validation is WAL-aware", () => {
  it("accepts a thread whose schema marker exists only in the WAL", () => {
    ...
    walOnly.exec("PRAGMA user_version = 13;");
    try {
...
describe("invalid thread-file candidates keep their exact classification", () => {
  ...
  const callerCases: Array<[string, (path: string) => void, string]> = [
    ...
    [
      ...
      "schema version 99, expected 1..13",
    ],
    [
      "missing-table",
      (path) => {
        const db = new DatabaseSync(path);
        db.exec("CREATE TABLE unrelated (x); PRAGMA user_version = 13;");
        db.close();
      },
    ...
    [
      ...
      (path) => {
        const db = new DatabaseSync(path);
        db.exec("CREATE TABLE thread_metadata (id INTEGER PRIMARY KEY, thread_id TEXT); PRAGMA user_version = 13;");
        db.close();
      },
