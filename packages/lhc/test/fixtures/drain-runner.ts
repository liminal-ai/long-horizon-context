...
// Argument: one JSON config — { threadPath, leaseMs, holdMs, holdFrom, exitAfterMs? }.
...
//   - with exitAfterMs, exits cleanly (process.exit(0)) that long after the
//     held handler starts, mid-derivation, like a one-shot ending its turn;
  ...
  registerTestWorkHandlers(sdk, double, {
    onHandlerStart: async (item) => {
      ...
      if (started >= config.holdFrom) {
        if (config.exitAfterMs !== undefined) {
          setTimeout(() => {
            process.stdout.write("CLEAN_EXIT\n");
            process.exit(0);
          }, config.exitAfterMs);
        }
