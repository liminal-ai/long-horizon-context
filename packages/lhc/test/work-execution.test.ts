...
describe("TC-1.8 / AC-1.9: failed work is terminal and the drain continues", () => {
  it("the first failure marks failed, deletes the row, and runs the next item immediately", async () => {
    const double = createInferenceCallbacksDouble();
    const sdk = manualSdk(double);
    const { filePath } = await newThread();
    await send(sdk, filePath, [validEvent("user_prompt"), validEvent("turn_end")]);
    ...
    expect(failed?.reason).toBe("scripted failure (smoothPrompt)");
...
describe("claim ownership fencing", () => {
  function deferredMessageSdk(now: { ms: number }) {
    ...
    const runs: Array<{ item: { workItemId: string }; resolve: (scripted: Scripted) => void }> = [];
...
describe("completion exactness", () => {
  ...
  it("a stale queued terminal failure deletes owned work without stamping the newer derivation", async () => {
    ...
    double.failKind("prompt_smoothing", 1, { reason: "stale terminal failure" });
    const report = await drain(sdk, filePath);
    expect(report.ran[0]).toMatchObject({
      workItemId: "w-m1-prompt_smoothing-v1",
      disposition: "failed_terminal",
...
describe("sync derive collision policy", () => {
  it("messages.derive is a bounded inline attempt and does not consume queued work", async () => {
    ...
    expect(liveDetail(filePath)).toEqual([
      expect.objectContaining({ workItemId: "w-m1-prompt_smoothing-v1", status: "queued" }),
    ]);
    expect(readDerivedForms(filePath)[0]).toMatchObject({ state: "pending", sourceVersion: 1 });
  });

  it("messages.derive attempts once and leaves no queued work on failure", async () => {
    const double = createInferenceCallbacksDouble();
    double.failKind("prompt_smoothing", 1, { reason: "timeout: inline attempt" });
    const sdk = manualSdk(double);
    const { filePath } = await newThread();
    await send(sdk, filePath, [validEvent("user_prompt")]);
