...
**Choice:** One shared set of state names for derived artifacts, pinned once at the first derivation tech design. The names must distinguish: not-yet-derived, usable, retryable failure, terminal failure, and blocked on damaged source. What a state means for a specific artifact stays the owning domain's call.
...
- **The record is append-plus-edit only.** No feature destroys events or messages; the only record mutation is the explicit edit operation with clear-and-regenerate semantics.
