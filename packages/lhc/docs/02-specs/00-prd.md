...
Out: running queued work (Feature 2), assembling views (Feature 3), editing the record (Feature 4).
...
The asynchronous half: queued work runs, derived artifacts land with explicit state, and the record's derived layer stays repairable.
...
**Scenario 3: Old tool results age out of full visibility.** As the recent tail grows, older tool results become eligible for abbreviation, then activate in batches — every few turns or on budget pressure — so the prompt prefix changes in planned steps rather than every turn.
...
## Feature 4: Inspection and Curation

Seeing and fixing the record: read operations across the history, reports across the thread, and manual edits.

### Scope

In: listing, viewing, and searching messages; the message edit operation; inspect reports — thread size and composition, turn and chunk counts, derivation health, current view contents and load cost.

Out: write paths other than message editing, repair execution (owned by Feature 2's domain surfaces, reported on here).
...
**Scenario 3: An operator repairs a confusing stretch.** A message contains something wrong or noisy. The operator edits it; the record updates, dependent derivations clear and re-queue per Feature 2, and inspect shows the rebuild's progress.

- AC-4.4: Message edits go through the messages surface, preserve the thread's coherence invariants, and trigger clear-and-regenerate on dependents
...
**The record is never destroyed.** Views, summaries, and abbreviations are derived; the full event and message record they derive from persists. The only mutation of the record is the explicit message edit operation. Rationale: recoverability is the product. Any feature that trades record fidelity for convenience breaks the premise.
...
- **M4 — Curation and hardening (Feature 4):** inspection and editing complete; SDK exercised end-to-end at its integration points. Feedback gate: run the full SDK surface the way the PI extension will.
...
flowchart LR
  ...
  E2 --> E4[Epic 4: Inspection and Curation]
  E3 --> E4
```

Epic 1 has no dependencies and proves the hot path. Epic 2 consumes Epic 1's queued work. Epic 3 consumes Epic 2's artifacts. Epic 4 reads everything and lands last, though its message read/search portion could start after Epic 1. Likely splits if epics run large: Epic 2 into turn/chunk derivation versus edit-and-repair; Epic 3 into assembly/compact versus rendering/readiness. That yields four to six epics.
