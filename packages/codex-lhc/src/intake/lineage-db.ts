...
export function lineageWriteFailureMessage(cause: unknown): string {
  ...
  return `[codex-lhc] lineage write failed (continuing): ${message}`;
}

export function lineageReadFailureMessage(cause: unknown): string {
  const message = cause instanceof Error ? cause.message : String(cause);
  return `[codex-lhc] lineage read failed (continuing): ${message}`;
}
...
function initLineageSchema(db: DatabaseSync): void {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS codex_session_lineage (
  ...
  `);
  db.exec(`
    CREATE TABLE IF NOT EXISTS codex_thread_signatures (
      thread_id TEXT NOT NULL,
...
export function lookupThreadForSession(dbPath: string, sessionId: string, deps: LineageDbDeps = {}): string | undefined {
  ...
  withLineageDb(dbPath, deps, (db) => {
    const row = db
      .prepare("SELECT thread_id FROM codex_session_lineage WHERE rollout_session_id = ?")
...
export function newestSessionEntry(
  ...
  withLineageDb(dbPath, deps, (db) => {
    const row = db
      .prepare(
        "SELECT rollout_session_id, thread_id, updated_at FROM codex_session_lineage ORDER BY updated_at DESC LIMIT 1",
      )
...
async function walkNewestRolloutSession(
  dir: string,
  deps: Required<Pick<LineageDbDeps, "readdirFn" | "statFn">>,
  best: { sessionId: string; mtimeMs: number } | undefined,
): Promise<{ sessionId: string; mtimeMs: number } | undefined> {
  let names: string[];
  try {
    names = await deps.readdirFn(dir);
  } catch (cause) {
    if (isENOENT(cause)) return best;
    throw cause;
  }
  ...
  for (const name of names) {
    ...
    try {
      childStat = await deps.statFn(childPath);
    } catch {
      continue;
    }

    if (childStat.isDirectory()) {
      best = (await walkNewestRolloutSession(childPath, deps, best)) ?? best;
      continue;
    }
    ...
    if (best === undefined || childStat.mtimeMs > best.mtimeMs) {
      best = { sessionId, mtimeMs: childStat.mtimeMs };
    }
  }
...
export async function newestRolloutSessionId(
  sessionsRoot: string,
  deps: LineageDbDeps = {},
): Promise<string | undefined> {
  const merged = { ...defaultDeps(), ...deps };
  const best = await walkNewestRolloutSession(sessionsRoot, {
    readdirFn: merged.readdirFn!,
    statFn: merged.statFn!,
  }, undefined);
  return best?.sessionId;
}

export async function tryContinueThreadFromResumeLast(
  dbPath: string,
  sessionsRoot: string,
  deps: LineageDbDeps = {},
  ...
  const threadId = lookupThreadForSession(dbPath, best.sessionId, deps);
  if (threadId === undefined) return undefined;
  return { sessionId: best.sessionId, threadId };
}
...
export function recordSessionThread(
  ...
  withLineageDb(dbPath, deps, (db) => {
    ...
    try {
      db.prepare(
        "INSERT INTO codex_session_lineage (rollout_session_id, thread_id, updated_at) VALUES (?, ?, ?) ON CONFLICT(rollout_session_id) DO UPDATE SET thread_id = excluded.thread_id, updated_at = excluded.updated_at",
...
function trimThreadSignatures(db: DatabaseSync, threadId: string): void {
  db.prepare(
    `DELETE FROM codex_thread_signatures
     WHERE thread_id = ?
       AND seq NOT IN (
         SELECT seq FROM codex_thread_signatures
         WHERE thread_id = ?
         ORDER BY seq DESC
         LIMIT ?
       )`,
...
export function appendThreadSignatures(
  ...
  withLineageDb(dbPath, deps, (db) => {
    ...
    try {
      const maxRow = db
        .prepare("SELECT COALESCE(MAX(seq), 0) AS max_seq FROM codex_thread_signatures WHERE thread_id = ?")
        .get(threadId) as { max_seq: number };
      let nextSeq = maxRow.max_seq;
      const insert = db.prepare(
        "INSERT OR IGNORE INTO codex_thread_signatures (thread_id, signature, seq) VALUES (?, ?, ?)",
      );
...
export function loadThreadSignatures(dbPath: string, threadId: string, deps: LineageDbDeps = {}): string[] {
  ...
  withLineageDb(dbPath, deps, (db) => {
    const rows = db
      .prepare("SELECT signature FROM codex_thread_signatures WHERE thread_id = ? ORDER BY seq ASC")
...
export async function resolveCaptureThread(input: ResolveCaptureThreadInput): Promise<ResolveCaptureThreadResult> {
  ...
  if (threadId === undefined && input.resumeLast === true && input.sessionsRoot !== undefined) {
    const continued = await safeLineageReadAsync(
      logError,
      () => tryContinueThreadFromResumeLast(dbPath, input.sessionsRoot!, input.lineageDeps),
      undefined,
    );
  ...
  if (threadId !== undefined) {
    log(`codex-lhc: continuing thread ${threadId} for session ${input.sessionId}`);
  ...
  if (!created.ok) {
    throw new Error(`codex-lhc thread create failed: ${created.error.reason}`);
  }
  const newThreadId = "threadId" in created.value ? created.value.threadId : "";
  if (newThreadId === "") {
    throw new Error("codex-lhc thread create failed: missing threadId");
  }
