...
describe("createClaudeCliModelCall", () => {
  it("writes user content to stdin and maps success stdout", async () => {
    ...
    const harness = fakeCall({ CODEX_LHC_FAKE_MODE: "stdin-file", CODEX_LHC_FAKE_STDIN_FILE: stdinFile });
  ...
  it("classifies auth stderr", async () => {
    const harness = fakeCall({ CODEX_LHC_FAKE_MODE: "auth" });
  ...
  it("classifies rate-limit stderr", async () => {
    const harness = fakeCall({ CODEX_LHC_FAKE_MODE: "rate_limit" });
    const result = await harness.call(baseInput);
    harness.restore();
  ...
  it("classifies generic nonzero exit as other", async () => {
    const harness = fakeCall({ CODEX_LHC_FAKE_MODE: "generic" });
    const result = await harness.call(baseInput);
    harness.restore();
  ...
  it("kills on timeout and returns timeout failure", async () => {
    const harness = fakeCall({ CODEX_LHC_FAKE_MODE: "sleep", CODEX_LHC_FAKE_SLEEP_MS: "5000" }, { timeoutMs: 80 });
    const result = await harness.call(baseInput);
    harness.restore();
  ...
  it("limits concurrent child processes", async () => {
    ...
    const harness = fakeCall(
      {
        CODEX_LHC_FAKE_MODE: "concurrency",
        CODEX_LHC_FAKE_COUNTER_FILE: counterFile,
        CODEX_LHC_FAKE_SLEEP_MS: "300",
      },
      { maxConcurrency: 2 },
    );
  ...
  it("returns empty stdout for adapter empty_output classification", async () => {
    const harness = fakeCall({ CODEX_LHC_FAKE_MODE: "empty" });
    const result = await harness.call(baseInput);
    harness.restore();
  ...
  it("rejects non cc-cli provider", async () => {
    const harness = fakeCall({ CODEX_LHC_FAKE_MODE: "success" });
  ...
  it("returns slot timeout without spawning when semaphore wait exceeds timeoutMs", async () => {
    ...
    Object.assign(process.env, {
      CODEX_LHC_FAKE_MODE: "hold-slot",
      CODEX_LHC_FAKE_SENTINEL_FILE: sentinelPath,
      CODEX_LHC_FAKE_SLEEP_MS: "200",
    });
    ...
    for (const key of ["CODEX_LHC_FAKE_MODE", "CODEX_LHC_FAKE_SENTINEL_FILE", "CODEX_LHC_FAKE_SLEEP_MS"]) {
      if (prior[key] === undefined) delete process.env[key];
      else process.env[key] = prior[key];
    }
  ...
  it("survives stdin EPIPE when child exits before consuming stdin", async () => {
    const harness = fakeCall({ CODEX_LHC_FAKE_MODE: "immediate-exit" });
