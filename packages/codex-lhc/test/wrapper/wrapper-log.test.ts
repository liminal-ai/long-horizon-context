...
describe("createWrapperLog", () => {
  it("appends timestamped lines, counts only warnings, creates the directory", async () => {
    const dir = mkdtempSync(join(tmpdir(), "codex-lhc-wlog-"));
  ...
  it("derives warning count from the log file so a fresh instance still reports them", async () => {
    const dir = mkdtempSync(join(tmpdir(), "codex-lhc-wlog-durable-"));
  ...
  it("counts only warn records, not message-body tokens or continuation lines", async () => {
    const dir = mkdtempSync(join(tmpdir(), "codex-lhc-wlog-shape-"));
    const path = join(dir, "wrapper.log");
    const log = createWrapperLog(path);
...
describe("settleReceipts (panel settle decision)", () => {
  it("auto-dismisses (null) ONLY on a confirmed swap", () => {
    expect(settleReceipts(["compact view=v4"], { confirmed: true, dismissedForRespawn: true })).toBe(null);
  });

  it("keeps the panel for refusals, errors, no-ops, and status/stats", () => {
    expect(settleReceipts(["turn in progress — rerun when idle"], undefined)).toEqual([
      "turn in progress — rerun when idle",
    ]);
    expect(settleReceipts(["prune boundary 0 -> 0", "no-op"], undefined)).toEqual([
      "prune boundary 0 -> 0",
      "no-op",
    ]);
    expect(
      settleReceipts(["compact view=v4", "swap did not confirm"], {
        confirmed: false,
        dismissedForRespawn: false,
      }),
    ).toEqual(["compact view=v4", "swap did not confirm"]);
    expect(
      settleReceipts(["compact view=v4"], {
        confirmed: false,
        dismissedForRespawn: true,
        failurePanelShown: true,
      }),
    ).toBe(null);
  });
});
