...
use fixtures::{
    DERIVATION_TYPES, FAKE_MODEL_PREFIX, FAKE_PROVIDER_PREFIX, InferenceDerivationType,
...
async fn a_seeded_drain_exercises_all_seven_kinds_each_call_carrying_exactly_its_assigned_strings()
{
    ...
    for kind in InferenceDerivationType::ALL {
        ...
        for input in &lane {
            ...
            assert_eq!(input.model, assignments[*kind].model);
        }
...
async fn a_three_lane_mixed_config_routes_each_call_by_item_kind_with_no_cross_kind_bleed() {
    ...
    let lanes = HashMap::from([
        (InferenceDerivationType::SmoothedPrompt, "lane-alpha"),
        (InferenceDerivationType::ToolResultSummary, "lane-beta"),
        (
            InferenceDerivationType::DetailedTurnCompression,
            "lane-beta",
        ),
        (InferenceDerivationType::ChunkSummaryBrief, "lane-gamma"),
    ]);
    let mut overrides = HashMap::new();
    for (kind, provider) in &lanes {
        overrides.insert(
            *kind,
    ...
    for kind in InferenceDerivationType::ALL {
        ...
        let calls: Vec<_> = log_snapshot
            .iter()
            .filter(|input| input.model == assignments[*kind].model)
            .cloned()
            .collect();
        assert!(!calls.is_empty());
        for input in &calls {
            assert_eq!(input.provider, lanes[kind]);
        }
        let ready: Vec<_> = forms
            .iter()
            .filter(|form| form.derivation_type == kind_name && form.state.as_str() == "ready")
