...
Rust-adapted):
  ...
  passed     transitional mode (real Phase-2 todos remain): exact-name
             ...
             639 passed / 0 notimpl / 15 ignored / 0 wrong / 0 suspicious.
...
def classify() -> int:
    ...
    if final_mode:
        ...
        # contract/parity/hardening tests → 639 (584 + 55).
        if (
            len(buckets["passed"]) != 639
            ...
            print(
                ...
                "passed=639 notimpl=0 ignored=15 wrong=0 suspicious=0"
            )
            return 1
    print("GATE PASS")
    return 0
