...
Rust-adapted):
  ...
  passed     transitional mode (real Phase-2 todos remain): exact-name
             ...
             565 passed / 0 notimpl / 15 ignored / 0 wrong / 0 suspicious.
...
def classify() -> int:
    ...
    if final_mode:
        ...
        # with 565 names or a broad/prefix wildcard.
        # 565 = 563 (R6 footer fix-up) + 2 retrieval id-cap legs.
        if (
            len(buckets["passed"]) != 565
            ...
            print(
                ...
                "passed=565 notimpl=0 ignored=15 wrong=0 suspicious=0"
            )
            return 1
    print("GATE PASS")
    return 0
