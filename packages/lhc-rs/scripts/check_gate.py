...
Rust-adapted):
  ...
  passed     transitional mode (real Phase-2 todos remain): exact-name
             ...
             508 passed / 0 notimpl / 15 ignored / 0 wrong / 0 suspicious.
...
def classify() -> int:
    ...
    if final_mode:
        ...
        # with 508 names or a broad/prefix wildcard.
        # 508 = 496 (pre-propagation baseline) + 12 R1 empty-thinking-husk suite.
        if (
            len(buckets["passed"]) != 508
            ...
            print(
                ...
                "passed=508 notimpl=0 ignored=15 wrong=0 suspicious=0"
            )
            return 1
    print("GATE PASS")
    return 0
