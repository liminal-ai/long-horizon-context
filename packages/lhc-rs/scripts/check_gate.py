...
Rust-adapted):
  ...
  passed     transitional mode (real Phase-2 todos remain): exact-name
             ...
             726 passed / 0 notimpl / 15 ignored / 0 wrong / 0 suspicious.
...
def classify() -> int:
    ...
    if final_mode:
        ...
        # 726 (584 + 140). Keep this exact-count ledger in lockstep with
        # cargo --features test-util.
        if (
            len(buckets["passed"]) != 726
            ...
            print(
                ...
                "passed=726 notimpl=0 ignored=15 wrong=0 suspicious=0"
            )
            return 1
    print("GATE PASS")
    return 0
