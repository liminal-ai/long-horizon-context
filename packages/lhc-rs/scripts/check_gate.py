...
Rust-adapted):
  ...
  passed     transitional mode (real Phase-2 todos remain): exact-name
             ...
             516 passed / 0 notimpl / 15 ignored / 0 wrong / 0 suspicious.
...
def classify() -> int:
    ...
    if final_mode:
        ...
        # with 516 names or a broad/prefix wildcard.
        # 516 = 508 (post-R1) + 8 R2 thinking-signature suite.
        if (
            len(buckets["passed"]) != 516
            ...
            print(
                ...
                "passed=516 notimpl=0 ignored=15 wrong=0 suspicious=0"
            )
            return 1
    print("GATE PASS")
    return 0
