...
Rust-adapted):
  ...
  passed     transitional mode (real Phase-2 todos remain): exact-name
             ...
             526 passed / 0 notimpl / 15 ignored / 0 wrong / 0 suspicious.
...
def classify() -> int:
    ...
    if final_mode:
        ...
        # with 526 names or a broad/prefix wildcard.
        # 526 = 516 (post-R2) + 10 R3 turn-message-labels suite.
        if (
            len(buckets["passed"]) != 526
            ...
            print(
                ...
                "passed=526 notimpl=0 ignored=15 wrong=0 suspicious=0"
            )
            return 1
    print("GATE PASS")
    return 0
