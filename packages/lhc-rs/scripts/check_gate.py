...
Rust-adapted):
  ...
  passed     transitional mode (real Phase-2 todos remain): exact-name
             ...
             654 passed / 0 notimpl / 15 ignored / 0 wrong / 0 suspicious.
...
def classify() -> int:
    ...
    if final_mode:
        ...
        # contract/parity/hardening tests → 654 (584 + 55).
        if (
            len(buckets["passed"]) != 654
            ...
            print(
                ...
                "passed=654 notimpl=0 ignored=15 wrong=0 suspicious=0"
            )
            return 1
    print("GATE PASS")
    return 0
