//! SHA-256 hex digests matching Node `createHash("sha256").update(...).digest("hex")`.
//!
//! Used by prepared-compact source fingerprints (LIM-61/63A). Pure FIPS 180-4 —
//! no external crypto dependency.

/// Hex-encoded SHA-256 of raw bytes (lowercase).
