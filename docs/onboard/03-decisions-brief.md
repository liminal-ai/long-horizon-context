...
- **Compact never calls a model — assembly, not summarization.** It selects already-derived artifacts; missing material degrades down visible ladders or gaps, never derives inline. Canonical-record irregularity *skips and reports* the unplaceable record (raw rows untouched); derived-material damage *degrades*. Gap means nothing usable exists; degraded means real fallback content shown with a marker. [VIEW-2, 7, 13; VOCAB-11]
