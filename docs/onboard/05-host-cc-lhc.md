...
**Receipts.** A settled command puts its receipt lines into the panel above a fresh prompt, screen still held. The panel owns the alternate screen, so receipts are immune to the main-screen TUI's repaints by construction (raw prints are overwritten within a frame while a turn streams — rig-verified back when the modal shared the canvas). One keypress dismisses: leave the alt screen, then flush.
...
- **Resume-log scribble** — the stderr resume log printed at injection time lands on the alt-screen panel for a moment; the next panel redraw clears it and it vanishes entirely on dismiss.
