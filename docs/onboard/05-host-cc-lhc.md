...
Last verified against code: 2026-07-06. Precedence when facts disagree: code, then README, then [03-decisions-brief](03-decisions-brief.md), then this doc; see also the [decision registry](../decision-registry.md).
...
**Arming.** Interception arms only on a fresh line, when a `/` (`0x2f`) is typed into an empty input box. At that point the `/` is echoed to the screen, withheld from the PTY, and the shadow buffer becomes `"/"`. Freshness is a **shadow count of Claude Code's input box** (`shadowLen`), not a boolean: printable bytes and UTF-8 lead bytes increment it, backspace/DEL (`0x7f`/`0x08`) decrement with floor 0, Enter/ctrl-U/ctrl-C zero it, and kitty CSI-u Enter/Esc/Backspace (`13u`/`27u`/`127u`, press and repeat but not release) mirror the same moves — claude pushes kitty disambiguate mode (`ESC[>1u`), so Esc arrives as `27u` in kitty-protocol terminals. Bracketed-paste markers don't count; paste content does, and pasted newlines count as literal box content rather than submits. A `/lhc` typed mid-line (count > 0) is **not** intercepted. A **bare Esc zeroes the count unconditionally** — a deliberate recovery affordance, not box mirroring: claude 2.1.202 does *not* clear its input box on Esc (probed live), and the box can also fill behind the count's back (up-arrow history recall is a passed-through CSI). Typing drift only overcounts ("won't arm until Esc"), but after an Esc-with-draft or a history recall a command can arm while a draft sits visibly in the box — the command dispatches, the draft stays put, and a later bare Enter submits that draft. Accepted trade for never being locked out of `/lhc-*`.
...
- **Escape cancels and re-arms** — a bare ESC while withholding erases the echo, drops the candidate command without dispatching it, and zeroes the shadow count, so an immediately retyped `/lhc-…` arms again.
