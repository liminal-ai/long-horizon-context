...
- Attempt 4 passed end to end in [Actions run 31695704439](https://github.com/liminal-ai/long-horizon-context/actions/runs/31695704439): native build/test on Blacksmith Windows x64 and GitHub Windows ARM64, npm artifact assembly, clean global npm install, allowlisted pinned Claude native installation, and cc-lhc PTY/version smoke on both architectures.
- Installed the green-run tarball on the pilot ARM64 workstation under user-local Node. Artifact ZIP SHA-256: `360da46896793370de7acd2ac516ae1b9202fc148a07be27916c4ee8fa451c33`; tarball SHA-256: `d76bc972c41bcb71df6fdad11c799c9b87f6b73b630bd61c048bf74c183b2d2f`.
- Workstation resolution is `C:\Users\leemoore\AppData\Local\Programs\node-v24.18.0-win-arm64\cc-lhc.cmd`, with Claude Code 2.1.231 at `C:\Users\leemoore\.local\bin\claude.exe`. `cc-lhc --version` and `cc-lhc --lhc-help` pass, and an interactive npm-installed wrapper/native-Claude PTY session is live in `C:\Users\leemoore\cc-lhc-dogfood`.
- Confirmed no compiler toolchain was added: `cl.exe` and `msbuild.exe` are absent; the `python.exe` entry is only Windows' zero-byte Store alias.
- Windows upgrade caveat: reinstalling while cc-lhc is running can leave npm's renamed `.cc-lhc-*` backup directory because the loaded `.node` file is locked (`EPERM`). The install succeeds, but users and managed deployment tools should close all cc-lhc sessions before upgrade and should treat cleanup warnings as actionable.
